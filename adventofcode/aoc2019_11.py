...
class Robot(Intcode):
    def __init__(self, memory, grid:dict, position:tuple, direction:Direction):
    ...
    def output_action(self, output):
        if self.output_is_color:
            self.grid[self.pos] = str(output)
        else:
            self.move(output)
        self.output_is_color = not self.output_is_color
...
robot = Robot(memory, grid, (0, 0), Direction.UP)
robot.run()
#print(len(robot.grid))

xs = [x for x, y in robot.grid.keys()]
minx = min(xs)
maxx = max(xs)
ys = [y for x, y in robot.grid.keys()]
miny = min(ys)
maxy = max(ys)
print(minx, maxx, miny, maxy)

image = [['⬛️' for x in range(maxx + 1)] for y in range(maxy + 1)]
for panel, color in robot.grid.items():
    if color == '1':
