data = '''ecl:hzl byr:1926 iyr:2010
...
def validate_passport(passport):
    if not (set(passport.keys()) - {'cid'} == fields - {'cid'}):
        print('a')
        return False
    ...
    if int(passport['eyr']) not in range(2020, 2031):
        ...
        return False
    height = passport['hgt']
    if not ((height.endswith('cm') and int(height[:-2]) in range(150, 194)) or (height.endswith('in') and int(height[:-2]) in range(59, 77))):
        print('e')
        return False
...
for passport in data:
    passport = [pair.split(':') for pair in passport.split()]
