...
for line in data.splitlines():
    words = line.split()
    preferences[words[0][0]][words[-1][0]] = int(words[3]) * (-1 if words[2] == 'lose' else 1)
for guest in 'ABCDEFGM':
    preferences['X'][guest] = 0
    preferences[guest]['X'] = 0

max_happiness = -99 * 9 * 2
for arrangement in permutations('ABCDEFGMX'):
    happiness = 0
    for i, j in enumerate(range(8), -1):
        guest1, guest2 = arrangement[i], arrangement[j]
        happiness += preferences[guest1][guest2] + preferences[guest2][guest1]
