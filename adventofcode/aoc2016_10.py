...
for line in data.splitlines():
    line = line.split()
    if line[0] == 'value':
        bots[line[-1]].add(line[1])
    else:
        instructions[line[1]] = (line[5:7], line[-2:])
#print(bots)
#print(sorted(instructions.items()))
    ...
    for bot, chips in bots.copy().items():
        ...
        if len(chips) == 2:
            ...
            if chips == {'61', '17'}:
                ...
                #running = False
                #break
            for chip, destination in zip(sorted(chips, key=int), instructions[bot]):
                if destination[0] == 'bot':
                    bots[destination[1]].add(chip)
                else:
                    outputs[destination[1]] = int(chip)
            bots[bot].clear()
