...
class Robot(aoc2019intcode.Intcode):
    def halt_action(self):
        return self.to_ascii(self.outputs)

robot = Robot(memory.copy())
view_str = robot.run()
rows = view_str.splitlines()
width = len(rows[0])
#print(view_str)
#print(rows)
...
for y, row in enumerate(rows[1:-1], 1):
    for x in range(len(row) - 1):
        ...
        #print(rows[y-1][x+1])
        #print(rows[y+1][x+1])
        if all(space in scaf for space in row[x:x+3] + rows[y-1][x+1] + rows[y+1][x+1]):
            alignment_parameter = (x+1) * y
            calibration_value += alignment_parameter
...
current = divmod(''.join(rows).index('^'), width)
...
while not dead_end:
    ...
    #print(rows[current[0]])
    #print(movements)
    #print(count)
    forward = point_add(current, directions[1])
    #print('forward', forward)
    try:
        #print(rows[forward[0]])
        #print(rows[forward[0]][forward[1]])
        
        if -1 not in forward and rows[forward[0]][forward[1]] == '#':
            current = forward
            #print('=')
            count += 1
            continue
        ...
        #print('e')
        pass
    movements.extend([str(count), ','])
    count = 0
    for (movement, rotation), direction in zip(zip('LR', (1, -1)), (directions[0], directions[2])):
        side = point_add(current, direction)
        #print('side', side)
        if rows[side[0]][side[1]] == '#':
            #print(movement)
        ...
        dead_end = True
movements = ''.join(movements)[:-1]
#print(movements)


instructions = \
'''A,C,B,A,B,A,B
L,6,R,12,L,4,L,6,R,6,L,6,R,12
L,6,L,10,L,10,R,6
R,6,L,6,R,12
n
'''
...
simplified_movements = movements.replace(',', '')
print(simplified_movements)
#test = simplified_movements[:16] + simplified_movements[18:]
#print(test)
#simplified_movements = test
found = False
for i in range(1, 11):
    if found:
        break
    a_func = simplified_movements[:i]
    for j in range(1, 11):
        b_func = simplified_movements[-j:]
        #print(a_func)
        #print(b_func)
        leftovers = set(re.split(rf'{a_func}|{b_func}', simplified_movements))
        #print(leftovers)
        leftovers.discard('')
        c_func = min(leftovers, key=len)
        try:
            c_func = re.fullmatch(r'(.+)\1*', c_func).group(1)
        except AttributeError:
            c_func = None
            continue
        if all(re.fullmatch(rf'({c_func})+', leftover) for leftover in leftovers) and len(c_func) <= 11:
            found = True
            break
#print(a_func)
#print(b_func)
#print(c_func)
main_routine = simplified_movements.replace(a_func, 'A').replace(b_func, 'B').replace(c_func, 'C')
#print(main_routine)

def format_instruction(instruction):
    for letter in 'LR':
        instruction = instruction.replace(letter, f',{letter},')
    for letter in 'ABC':
        instruction = instruction.replace(letter, f'{letter},')
    instruction = instruction.strip(',')
    return instruction + '\n'

instructions = ''.join(list(map(format_instruction, (main_routine, a_func, b_func, c_func)))) + 'n\n'
print(instructions)
...
memory[0] = 2
#print(robot.memory)
#print(robot.pointer, robot.relative_base)
#robot.memory[0] = 2
#print(robot.memory)
robot = Robot(memory)
view_str = robot.run(instructions)
#print(robot.pointer, robot.relative_base)
print(robot.outputs)
print(view_str)