data = '''Sue 1: children: 1, cars: 8, vizslas: 7
...
for n, line in enumerate(data.splitlines(), 1):
    skip = False
    for datum in line[line.index(':')+2:].split(', '):
        compound, count = datum.split(': ')
        count = int(count)
        if compound in ('cats', 'trees'):
            if count <= analysis[compound]:
                skip = True
                break
