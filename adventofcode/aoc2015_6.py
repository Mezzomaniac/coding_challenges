...
def get_selection(line):
    corners = [corner.split(',') for corner in re.findall(r'(\d+,\d+)', line)]
    corners = [tuple(int(n) for n in corner) for corner in corners]
    return slice(corners[0][1], corners[1][1]+1), slice(corners[0][0], corners[1][0]+1)

grid1 = np.zeros((1000, 1000), dtype=np.bool_)

for instruction in instructions:
    selection = get_selection(instruction)
    if 'on' in instruction:
        grid1[selection] = True
    elif 'off' in instruction:
        grid1[selection] = False
    else:
        grid1[selection] ^= 1
        
total_on = np.sum(grid1)
print(total_on)
...
for instruction in instructions:
    ...
    if 'on' in instruction:
        ...
        grid2[selection] -= 1
        np.clip(grid2, 0, np.inf, grid2)
    else:
        grid2[selection] += 2

total_brightness = np.sum(grid2)
