...
class Point:
    ...
    def __add__(self, other):
        return Point(self.x + other.x, self.y + other.y, self.steps + other.steps)
    ...
    def __repr__(self):
        return f'Point({self.x}, {self.y}, {self.steps})'

wires = [wire.split(',') for wire in data.splitlines()]

dir_dict = {'R': Point(1, 0, 1), 'U': Point(0, 1, 1), 'L': Point(-1, 0, 1), 'D': Point(0, -1, 1)}
...
for turn in wires[0]:
    ...
    for step in range(distance):
        current += dir_dict[direction]
        #current.steps += 1
        if current not in path1:
            path1.add(current)
    #path |= {current + dir_dict[direction] * step for step in range(1, distance + 1)}
    #current += dir_dict[direction] * distance
    #paths.append(path)
...
#times = {point.steps for point in intersections}
#print(min(distances))
#time = min(point1.steps + point2.steps for point1 in paths[0] for point2 in paths[1] if point1 == point2)
time = min(point1.steps + point2.steps for point1 in path1 for point2 in path2 if point1 == point2)
print(time)