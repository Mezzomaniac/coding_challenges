messages = '''abbbaaabbbbbbabaaaabbababbaabaabbbbaababbbbbaababbbaabbabbbabbababaaabaaaabaabbbaaababab
...
test_messages1 = '''ababbb
bababa
abbbab
aaabbb
aaaabbb'''

test_rules_data1 = '''0: 4 1 5
1: 2 3 | 3 2
2: 4 4 | 5 5
3: 4 5 | 5 4
4: "a"
...
test_messages2 = '''abbbbbabbbaaaababbaabbbbabababbbabbbbbbabaaaa
bbabbbbaabaabba
...
#messages, rules_data = test_messages2, test_rules_data2
...
for rule_data in rules_data.splitlines():
    rule_num, rule = rule_data.split(': ')
    if 'a' in rule or 'b' in rule:
        rules[rule_num] = rule[1]
        #parsed.add(rule_num)
    else:
        rules[rule_num] = f'( {rule} )'

rules['8'] = '( 42 )+'
rule11 = '( 42 {{{m}}} 31 {{{m}}}'
LETTERS = 'nopqrstuv'
for letter in LETTERS:
    rule11 += f' | 42 {{{{{{{letter}}}}}}} 31 {{{{{{{letter}}}}}}}'
rules['11'] = rule11 + ' )'

rule0 = rules['0']
while re.search('\d', rule0):
    update = []
    for part in rule0.split():
        #print(part)
        if not re.search('\d', part):
            update.append(part)
            continue
        #print(rules[part])
        update.append(rules[part])
    rule0 = ' '.join(update)
    #print(rule0)
rule0 = rule0.replace(' ', '')
#print(rule0)

copies = dict(zip('m' + LETTERS, range(1,11)))
rule0 = rule0.format(**copies)
#print(rule0)

print(sum(bool(re.fullmatch(rule0, message)) for message in messages.splitlines()))
