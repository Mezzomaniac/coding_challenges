data = '''fubrjhqlf-edvnhw-dftxlvlwlrq-803[wjvzd]
...
rooms = data.splitlines()

def check_room(room):
    name = room[:room.rindex('-')+1].replace('-', '')
    sector = int(room[room.rindex('-')+1: -7])
    checksum = room[-6: -1]
    most_common = ''.join(sorted(sorted(set(name)), key=lambda letter: Counter(name)[letter], reverse=True))[:5]
    #print(most_common)
    if most_common == checksum:
        return sector
    return 0

result = sum(check_room(room) for room in rooms)
print(result)

for room in rooms:
    sector = check_room(room)
    if sector:
        table = str.maketrans({a: abc[(abc.index(a) + sector) % 26] for a in abc})
        name = room[:room.rindex('-')].translate(table)
        if 'north' in name:
            print(name, sector)
