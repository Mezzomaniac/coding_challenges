...
rules = dict(rule.split(' contain ') for rule in data.splitlines())
#print(rules)
...
while not frontier.empty():
    ...
    for container, contents in rules.items():
        next = container.rstrip('s')
        if current in contents and next not in selected:
            #print(next, contents)
            selected.add(next)
            frontier.put(next)
            #print(selected)
...
while not frontier.empty():
    current, multiplier = frontier.get()
    print(current, multiplier, count)
    contents = rules[current].split(', ')
    print(contents)
    for item in contents:
        if item == 'no other bags.':
            continue
        n, *bag = item.split()
        next_multiplier = int(n) * multiplier
        count += next_multiplier
        next = ' '.join(bag).rstrip('s.') + 's'
        frontier.put((next, next_multiplier))
print(count)