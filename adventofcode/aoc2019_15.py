...
while not frontier.empty():
    ...
    droid = Droid(memory.copy())
    try:
        current_status = droid.run(moves_so_far[current])[-1]
    except IndexError:
        current_status = 1
    ...
    if not current_status:
        ...
        oxsys_location = current
        oxsys_distance = len(moves_so_far[oxsys_location])
        oxsys_memory = droid.memory
        oxsys_pointer = droid.pointer
        oxsys_relative_base = droid.relative_base
        print(oxsys_distance)
        break
    for movement in range(1, 5):
        next = point_add(current, directions[movement])
        moves = moves_so_far[current] + [movement]
        if next not in moves_so_far or len(moves) < len(moves_so_far[next]):
            moves_so_far[next] = moves
            frontier.put(next)

#print(moves_so_far[oxsys_location])

frontier = Queue()
moves_so_far = {}
start = oxsys_location
print(start)
moves_so_far[start] = []
frontier.put(start)
while not frontier.empty():
    current = frontier.get()
    droid = Droid(oxsys_memory.copy())
    droid.pointer = oxsys_pointer
    droid.relative_base = oxsys_relative_base
    try:
        current_status = droid.run(moves_so_far[current])[-1]
    except IndexError:
        current_status = 2
    ...
    for movement in range(1, 5):
        ...
        if next not in moves_so_far or len(moves) < len(moves_so_far[next]):
            moves_so_far[next] = moves
            frontier.put(next)
print(max(len(moves) for moves in moves_so_far.values()) - 1)