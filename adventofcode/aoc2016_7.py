...
def abba(string):
    for i in range(len(string) - 3):
        if string[i: i+2] == string[i+3: i+1: -1] and string[i] != string[i+1]:
            return True
    return False
...
def tls(ip):
    ...
    for string in hyper:
        if abba(string):
            return False
    return any(abba(string) for string in supernets)
...
def ssl(ip):
    ...
    hypers = ip[1::2]
    abas = aba(supernets)
    return any(string[1:] + string[1] in hyper for string in abas for hyper in hypers)

#print(sum(tls(ip) for ip in data.splitlines()))

print(sum(ssl(ip) for ip in data.splitlines()))
